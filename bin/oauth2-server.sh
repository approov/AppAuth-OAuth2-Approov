...
Show_Help() {
  ...
  --env-type      Sets the deployment enviroment type (defaults to remote):
                  $ ./oauth2 --env-type
                  $ ./oauth2 --env-type local
                  $ ./oauth2 --env-type remote
  ...
  build           Builds the docker image for an environment (defaults: --from master):
                  $ ./oauth2 build local
                  $ ./oauth2 --from-current build dev
                  $ ./oauth2 --from feature_branch build rc
                  $ ./oauth2 --from git_tag build prod

  copy-env        Copies the remote .env to ./.local/deploy (defaults: --env local):
                  $ ./oauth2 copy-env
                  $ ./oauth2 --env staging copy-env
                  $ ./oauth2 --env prod copy-env

  update-env      Updates the local env deploy file from the .env remote file (defaults: --env local):
                  $ ./oauth2 update-env
                  $ ./oauth2 --env staging update-env
                  $ ./oauth2 --env prod update-env

  deploy          Deploys a release (defaults: --env local, --env-type remote, --from master):
                  $ ./oauth2 --from-current deploy
                  $ ./oauth2 --env staging --from branch-name deploy
                  $ ./oauth2 --env staging --env-type remote --from branch-name deploy
                  $ ./oauth2 --env staging --env-type local --from branch-name deploy
                  $ ./oauth2 --env prod deploy

  logs            Tails the logs on the remote server:
                  $ ./oauth2 logs
                  $ ./oauth2 --env prod logs
  ...
  rsync           Synchronizes the source code with the remote env:
                  $ ./oauth2 --env dev rsync
                  $ ./oauth2 --env dev --release-tag 1 rsync
...
Setup_Configuration() {
  ...
  case "${RELEASE_ENV}" in
    "local" | "dev" | "test" | "staging" | "rc" )
      # The public domain will look like: rc1.oauth2.demo.approov.io
      TRAEFIK_PUBLIC_DOMAIN="${RELEASE_ENV}${RELEASE_TAG}".${APP_NAME}.${SERVER_PUBLIC_DOMAIN}
    ;;

    "prod" )
      # The public domain will look like: oauth2.demo.approov.io
...
Npm_Install_Local_Server() {
  if [ ! -d "./oauth2-adapter/node_modules" ]; then
...
Boot_Remote_Server() {
  ...
  case "${RELEASE_ENV}" in
    ...
    "" )
      Exit_With_Error "Please provide the environment to use, e.g ./oauth2 up dev or ./oauth2 up staging"
    ;;
...
Shutdown_Remote_Server() {
  ...
  case "${shutdown_env}" in
    ...
    "" )
      Exit_With_Error "Please provide the environment to use, e.g ./oauth2 down dev or ./oauth2 down staging"
    ;;
...
Rsync_To_Remote() {
  ...
  rsync -azP \
    --delete-after \
    --exclude oauth2-adapter/dist \
    --exclude oauth2-adapter/node_modules \
    oauth2-adapter "${REMOTE_USER}"@"${REMOTE_ADDRESS}":"${REMOTE_APP_DIR}"
}
...
Deploy_Release() {
  ...
  SSH_Remote_Execute "cd ${REMOTE_APP_DIR} && cat "${ENV_DEPLOY_FILE}" >> .env"
  ...
  SSH_Remote_Execute "cd ${REMOTE_APP_DIR} && sudo docker-compose logs --tail 200"
